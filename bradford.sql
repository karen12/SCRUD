...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 04-07-2015 a las 23:32:47
...
--
CREATE DATABASE IF NOT EXISTS `bradford` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;
USE `bradford`;
...
INSERT INTO `usuarios` (`id_usuario`, `nombre`, `correo`, `id_tipo`, `usuario`, `clave`, `pregunta`, `respuesta`) VALUES
(1, 'diego', 'diegoalarama@gmail.com', 1, 'diego', '7110eda4d09e062aa5e4a390b0a572ac0d2c0220', 'asd', 'asd'),
(2, 'qew', 'asdgg.e@gmail.com', 1, 'allsop', 'cfd3f37719dcbc721bfae510a3101dbb75c19e16', '', '');
...
ALTER TABLE `institucion`
 ADD PRIMARY KEY (`id_ins`), ADD UNIQUE KEY `tel_ins` (`tel_ins`), ADD UNIQUE KEY `correo_ins` (`correo_ins`), ADD UNIQUE KEY `director_ins` (`director_ins`), ADD UNIQUE KEY `id_ins` (`id_ins`), ADD UNIQUE KEY `nombre_ins` (`nombre_ins`), ADD UNIQUE KEY `direccion_ins` (`direccion_ins`), ADD UNIQUE KEY `objetivo_ins` (`objetivo_ins`), ADD UNIQUE KEY `mision_ins` (`mision_ins`), ADD UNIQUE KEY `vision_ins` (`vision_ins`), ADD UNIQUE KEY `principios_ins` (`principios_ins`), ADD UNIQUE KEY `id_redes_s_2` (`id_redes_s`), ADD UNIQUE KEY `valores` (`valores`), ADD UNIQUE KEY `id_usuario_2` (`id_usuario`), ADD KEY `id_usuario` (`id_usuario`), ADD KEY `id_redes_s` (`id_redes_s`);
